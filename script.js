...
function confirmFigmaOrder() {
  try {
    // Подготовить данные заказа для отправки в бота
    const orderData = {
      command: "create_order",
      country: figmaOrderState.country,
      full_name: figmaOrderState.deliveryType === "same" ? 
        figmaOrderState.commonRecipient.name : 
        figmaOrderState.products[figmaOrderState.activeProducts[0]].recipient?.name || "",
      telegram_username: figmaOrderState.deliveryType === "same" ? 
        figmaOrderState.commonRecipient.username : 
        figmaOrderState.products[figmaOrderState.activeProducts[0]].recipient?.username || "",
      delivery_type: figmaOrderState.deliveryType,
      common_recipient: figmaOrderState.deliveryType === "same" ? figmaOrderState.commonRecipient : null,
      items: []
    }
    ...
    figmaOrderState.activeProducts.forEach(productId => {
      ...
      if (product.link && product.price) {
        ...
        // Добавить данные получателя для индивидуальной доставки
        if (figmaOrderState.deliveryType === "different" && product.recipient) {
          item.recipient = {
            name: product.recipient.name,
            username: product.recipient.username,
            address: product.recipient.address
          }
        }
    ...
    if (window.Telegram && window.Telegram.WebApp) {
      ...
      alert("Заказ подготовлен! (Тестовый режим - данные выведены в консоль)")
    }
